...
% causes crash in octave 4 needs to be checked for syntax change
%if ismatrix(itr)==0,		%check if itr is matrix with cell structure
%	itr=cell2mat(itr);
%endif

ne = numel(itr);                % number of tensor elements
nd = ndims(itr);                % number of tensor dimensions, i.e. order of tensor
if (ne==3), nd = 1 endif         % order of tensor is 1 in case of a 3x1 or 1x3 vector
